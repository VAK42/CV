...
while True:
  ...
  frameHeightVal = resized.shape[0]
    ...
    for kp, conf, bbox, score in zip(lastResult.keypoints.xy, lastResult.keypoints.conf, lastResult.boxes.xyxy,
      ...
      if all(conf[i] > minTrackingConfidence for i in requiredKp):
        ...
        if torsoLength > 10:
          ...
          hipsRatio = midHp[1] / frameHeightVal
          ...
          for pid, person in trackedPersons.items():
            ...
            dist = math.hypot(avgPos[0] - prevAvg[0], avgPos[1] - prevAvg[1]) / frameHeightVal
          ...
          if bestId is not None:
            ...
            if tDiff > 0:
              dySh = (midSh[1] - person[1]) / frameHeightVal
              dyHp = (midHp[1] - person[3]) / frameHeightVal
  ...
  infoText = f"FPS: {fps:.2f}  |  Fall: {fallDisplay}  |  Res: {w}x{h}  |  Time: {ts}"
