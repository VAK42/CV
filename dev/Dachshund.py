...
while True:
  ...
  if len(futures) < 1:
    futures.append(executor.submit(lambda img: model.predict(img, imgsz=640, verbose=False, max_det=60, half=False)[0], sframe))
  for f in futures[:]:
    if f.done():
      try:
  ...
  if lastFrameForDraw is not None:
    cv2.putText(lastFrameForDraw, f"FPS: {fpsCount:.2f}", (10, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1)
