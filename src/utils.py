...
def draw_bounding_boxes(frame, detections, labels=None):
  for i, (_, detection) in enumerate(detections.iterrows()):
    ...
    color = (0, 255, 0)
    if labels and i < len(labels) and labels[i] == "Falling":
      color = (0, 0, 255)
    cv2.rectangle(frame, (int(x1), int(y1)), (int(x2), int(y2)), color, 2)
    if labels and i < len(labels):
      cv2.putText(frame, labels[i], (int(x1), int(y1) - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.6, color, 2)
  return frame

def log_movement(movement_log, frame_number, detected_objects):
  with open(movement_log + ".log", "a") as log:
    log.write(f"{frame_number}:{len(detected_objects)}\n")
  with open(movement_log + ".csv", "a") as csv:
    for (x, y) in detected_objects:
      csv.write(f"{frame_number},{x},{y}\n")
